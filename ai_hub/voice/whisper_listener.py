...
def voice_process(command_queue, stop_event, clients_event=None):
    ...
    try:
        ...
        print("[Voice] Tearing down microphone...", flush=True)
        # We don't need Explicit `stream.close()` here because the `with` block 
        # manages it, but making sure the Try-Finally prints confirmation.
